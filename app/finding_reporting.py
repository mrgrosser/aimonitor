...
import hashlib
import hmac
from app.export_safety import SafeCSVWriter, csv_cell
import io
import json
import os
import smtplib
...
def findings_summary(rows: list[dict[str, Any]], include_named_users: bool = False) -> dict[str, Any]:
    ...
    details = [{"id":r.get("id") if include_named_users else "finding-" + hmac.new(os.getenv("SESSION_SECRET", "development-only-secret-change-me").encode(), str(r.get("id") or "").encode(), hashlib.sha256).hexdigest()[:24],"created_at":r.get("created_at"),"severity":r.get("risk"),"score":r.get("risk_score"),
        "category":", ".join(x.get("id","") for x in r.get("risk_factors",[])),"status":r.get("status","new"),
        "surface":r.get("surface"),"title":r.get("title") if include_named_users else "Policy finding: " + ", ".join(x.get("id", "unknown") for x in r.get("risk_factors", [])),"identity":_identity(r,include_named_users),
...
def findings_csv(report: dict[str, Any], actor: str = "", version: str = "", filters: dict[str, str] | None = None) -> bytes:
    out=io.StringIO(); meta=SafeCSVWriter(csv.writer(out)); meta.writerow(["JO AI Monitor Compliance Findings Report"]); meta.writerow(["Generated at",datetime.now(timezone.utc).isoformat()]); meta.writerow(["Generated by",actor]); meta.writerow(["Version",version]); meta.writerow(["Filters",json.dumps(filters or {},sort_keys=True)]); meta.writerow([])
    ...
    writer.writeheader(); writer.writerows({key:csv_cell(value) for key,value in row.items()} for row in report["findings"]); return out.getvalue().encode("utf-8-sig")
...
def findings_pdf(report: dict[str, Any], actor: str, version: str, filters: dict[str, str]) -> bytes:
    ...
    story=[Paragraph("JO AI Monitor - Compliance Findings Report",styles["Title"]),Paragraph(f"Generated {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} by {html.escape(actor)} - v{html.escape(version)}",styles["BodyText"]),
        Paragraph(html.escape(f"Range: {filters.get('date_from') or 'All'} through {filters.get('date_to') or 'All'} | Severity: {filters.get('risk','all')} | Surface: {filters.get('surface','all')}"),styles["BodyText"]),Spacer(1,10)]
    ...
    rows=[["Severity","Score","Date","Surface","Finding","Identity"]]+[[x["severity"],x["score"],str(x["created_at"])[:10],x["surface"],Paragraph(html.escape(str(x["title"])),styles["BodyText"]),x["identity"]] for x in report["findings"]]
