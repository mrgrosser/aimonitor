import csv
from app.export_safety import SafeCSVWriter
import hashlib
import hmac
import html
import io
...
def list_usage_periods() -> list[dict[str, Any]]:
    ...
    for period,payload,name,digest,created,actor in rows:
        ...
        result.append({"period":period,"source_name":name,"source_hash":digest,"imported_at":created,"imported_by":actor,
            "copilot_interactions":summary.get("copilot_interactions","Unavailable"),"claude_requests":summary.get("claude_requests","Unavailable"),
            "claude_usage_spend":summary.get("claude_usage_spend",0)})
    return result


def usage_csv(data: dict[str, Any], actor: str = "", version: str = "") -> bytes:
    out=io.StringIO(); writer=SafeCSVWriter(csv.writer(out)); writer.writerow(["JO AI Monitor Usage Report",data.get("period","")]); writer.writerow(["Generated at",datetime.now(timezone.utc).isoformat()]); writer.writerow(["Generated by",actor]); writer.writerow(["Version",version]); writer.writerow(["Source",data.get("source","")]); writer.writerow([])
