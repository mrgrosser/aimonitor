...
class FindingReportingTests(unittest.TestCase):
    ...
    def test_restricted_report_omits_prompt_titles_and_embedded_user_ids(self):
        self.rows[0].update(id="m365:person@example.com:request",title="Medical record for person@example.com")
        report=findings_summary(self.rows)
        self.assertNotIn("person@example.com",str(report))
        self.assertNotIn("Medical record",str(report))
        self.assertEqual(report["findings"][0]["id"],findings_summary(self.rows)["findings"][0]["id"])

    def test_pdf_treats_markup_as_literal_text(self):
        self.rows[0]["title"]="<img src='missing-file'> & <b>unfinished"
        result=findings_pdf(findings_summary(self.rows,True),"<tester>","v1",{"surface":"<b>"})
        self.assertTrue(result.startswith(b"%PDF"))

    def test_csv_formula_text_is_neutralized(self):
        import csv,io
        self.rows[0]["title"]='=HYPERLINK("https://example.com")'
        output=findings_csv(findings_summary(self.rows,True),actor="@actor")
        rows=list(csv.reader(io.StringIO(output.decode("utf-8-sig"))))
        self.assertIn(["Generated by","'@actor"],rows)
        self.assertIn("'"+self.rows[0]["title"],rows[-2])
