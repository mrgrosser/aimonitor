...
class RedirectTests(unittest.IsolatedAsyncioTestCase):
    async def test_download_redirect_does_not_forward_token(self):
        for host in ('reports.office.com','reportsncu.office.com'):
            downloads=[]
            ...
            async with httpx.AsyncClient(transport=httpx.MockTransport(lambda r:httpx.Response(302,headers={'location':f'https://{host}/data/download/example'}))) as client:
